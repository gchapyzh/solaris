...
export default {
  ...
  methods: {
    ...
    async hireSpecialist (specialist) {
        ...
        if (this.carrier.specialistId && !await this.$confirm('Replace Specialist', `The carrier already has a [${this.carrier.specialist.name}] hired, are you sure you want to replace it?`)) {
            return
        }
